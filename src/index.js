...
app.get("/Getdata", (req, res) => {
  EmployeeModel.find({})
    .select({ _id: 0, EmpId: 1, EmpName: 1, EmpDesignation: 1, EmployeeDQ: 1 })
    .then((data) => {
      res.send(data);
    });
});
app.get("/Getdata/:id", (req, res) => {
  EmployeeModel.findOne({ EmpId: { $eq: req.params.id } }, function (
    err,
    data
  ) {
    if (err) {
...
app.delete("/DeleteEmployee/:id", (req, res) => {
  EmployeeModel.findOneAndRemove({ EmpId: { $eq: req.params.id } }, function (
    err,
    data
  ) {
    if (err) {
